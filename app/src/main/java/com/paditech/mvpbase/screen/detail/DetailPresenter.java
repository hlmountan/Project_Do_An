...
//get a list of installed apps.
    ...
    public void getUserCmt(String appid, boolean isFirebase) {
        ...
        if (isFirebase)
            FirebaseDatabase.getInstance().getReference().child("cmt").orderByChild("appid").addValueEventListener(new ValueEventListener() {
                @Override
                public void onDataChange(DataSnapshot dataSnapshot) {
                    if (dataSnapshot.getValue() != null){
            ...
            else APIClient.getInstance().execGet("https://apprevi.com/api/apk/review-daily?appid=" + appid + "&time=7&star=5&language=en-US&page=1&size=6", null, new ICallBack() {
            @Override
            public void onErrorToken() {
